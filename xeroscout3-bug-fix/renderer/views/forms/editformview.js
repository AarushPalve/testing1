...
import { UndoStackEntry } from "./undo.js";
...
export class XeroEditFormView extends XeroView {
    constructor(app, type) {
        ...
        this.instance_id_ = -1;
        this.keybindings_ = new KeybindingManager();
        this.section_pages_ = [];
        this.dragging_ = 'none';
        this.last_select_index_ = -1;
        this.last_select_point_ = new XeroPoint(-100, -100);
        this.select_group_ = [];
        this.undo_stack_ = [];
        ...
        this.dirty_ = false;
        this.changing_ = [];
        this.base_ = new XeroPoint(0, 0);
        this.cursor_ = new XeroPoint(0, 0);
        this.context_menu_cursor_ = new XeroPoint(0, 0);
        this.ignore_resize_ = false;
        this.show_mouse_movement_ = false;
    ...
    shiftSectionRight(ev) {
        this.moveSection(false);
    }
    selectFurtherNext(ev) {
    ...
    unlockLockedControlsUnderCursor() {
        var _a;
        if (((_a = this.tabbed_ctrl_) === null || _a === void 0 ? void 0 : _a.selectedPageNumber) !== -1) {
    ...
    insertSectionPage(section, index) {
        let page = this.createSectionPageObject(section);
    ...
    deleteSectionByPage(pageno, save = true) {
    ...
    setTargetTablet(tablet, setModified = true) {
    ...
    //
    targetTabletChanged(setModified = true) {
    ...
    dialogClosed(ctrl, olditem, changed) {
        if (changed) {
            let arg = {
                formctrl: ctrl,
                olditem: olditem,
    ...
    testMoveItem(frmctrl, dx, dy, dw, dh, original) {
        if (frmctrl) {
    ...
    testMoveSelectedItems(dx, dy, dw, dh, original) {
        for (let frmctrl of this.selected_ctrls_) {
    ...
    deleteControls(ctrls, save = true) {
        let deled = [];
    ...
    draggingResizingControls(event) {
    ...
    selectInBox(start, end, add = false) {
        if (this.tabbed_ctrl_.selectedPageNumber !== -1) {
    ...
    mouseDown(event) {
        ...
        if (!XeroWidget.isChildOf(this.getFormDiv(), event.target)) {
            return;
        }
        event.preventDefault();
        event.stopPropagation();
        ...
        if (ctrls.length === 0) {
            if (this.selected_ctrls_.length > 0) {
                ...
                //
                if (XeroWidget.isChildOf(this.elem, event.target)) {
    ...
    renameSectionInternal(name, page, save = true) {
        if (this.form_) {
    ...
    sectionNameDialogDone(changed) {
        if (changed && this.form_) {
            let newname = this.edit_dialog_.enteredName;
    ...
    moveSectionInternal(pageno, left, save = true) {
    ...
    undo(ev) {
        ...
        if (undo.oper === 'add' && undo.obj === 'control') {
            this.deleteControls(undo.item, false);
        }
...
XeroEditFormView.instance_counter_ = 0;
XeroEditFormView.kSelectSameSpot = 5;
XeroEditFormView.kDefaultMiddleText = 'Right Click For Menu, F1 for keybindings';
XeroEditFormView.kCustomTabletName = 'Custom';
XeroEditFormView.moveControlAmount = 1;
XeroEditFormView.shiftMoveControlAmount = 10;
XeroEditFormView.ctrlMoveControlAmount = 50;
XeroEditFormView.shiftCtrlMoveControlAmount = 250;
//# sourceMappingURL=editformview.js.map