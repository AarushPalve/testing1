...
const xerogeom_js_1 = require("./xerogeom.js");
class RulesEngine extends events_1.default {
    constructor(form) {
        super();
        this.interval_ = null;
    ...
    findInterectingControls(ctrl, filter) {
        ...
        let bounds = new xerogeom_js_1.XeroRect(ctrl.x, ctrl.y, ctrl.width, ctrl.height);
        for (let c of page.items) {
            if (c === ctrl) {
                continue; // skip self
            }
            let cBounds = new xerogeom_js_1.XeroRect(c.x, c.y, c.width, c.height);
